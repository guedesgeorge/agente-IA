...
def get_usuario_atual(credentials = None):
    if not credentials:
        return None
...
class ChatRequest(BaseModel):
    ...
    use_web_search: bool = True        # ← NOVO: busca na internet
    usuario: str = "advogado"          # identifica qual dos 5 usuários está usando
    history: list = []

class ChatResponse(BaseModel):
    answer: str
    sources: list[dict]                # docs do escritório usados
    web_searches: list[dict]           # ← NOVO: buscas web realizadas
    session_id: str
...
async def upload_document(
    ...
    # Identifica usuário se autenticado
...
async def listar_documentos_usuario(
    ...
    """Lista apenas os documentos enviados pelo usuário logado."""
...
async def deletar_documento_usuario(
    ...
    if not user_id:
        raise HTTPException(401, "Não autenticado")
    # Verifica se o doc pertence ao usuário
...
def export_docx(req: ExportRequest):
    """Converte texto em .docx formatado e retorna para download."""
    try:
        ...
        for line in lines:
            ...
            # Detecta títulos (linhas com # ou numeradas tipo "1. OBJETO")
            if line.startswith('#'):
                ...
                # Linha toda em maiúsculo = título de seção
                p = doc.add_paragraph()
                run = p.add_run(line)
                run.bold = True
                run.font.size = Pt(11)
...
async def alterar_senha(req: AlterarSenhaRequest, credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    # Verifica senha atual (ou permite definir se ainda não tem)
...
async def cadastrar_email(email: str, credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Usuário cadastra seu email para recuperação."""
    user_id = get_usuario_atual(credentials)
    if not user_id:
        raise HTTPException(401, "Não autenticado")
    users = carregar_usuarios()
...
# ── ROTAS DE HISTÓRICO POR USUÁRIO ──────────────────────────────────────────
